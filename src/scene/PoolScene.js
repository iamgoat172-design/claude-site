...
// setProgress(0..1) — 8 фаз постройки; setHeroBlend(1..0) — красивый переход
// от beauty-состояния героя к старту постройки (замер).
...
// габарит чаши в юнитах сцены (≈ CLASSIC 8,5×3,7)
const POOL_W = 6.4;
const POOL_D = 2.9;
const POOL_H = 1.35;
...
export class PoolScene {
  constructor(canvas, { glbUrl = '', reduced = false } = {}) {
    ...
    this.scene.fog = null; // без дымки: урок iter8 — перекрут топит чёрный в муть

    this.camera = new THREE.PerspectiveCamera(38, 1, 0.1, 100);

    // env — приглушённый (0.22): достаточно для материалов, не серит фон
  ...
  _buildSurvey() {
    ...
    const sweepGeo = new THREE.CircleGeometry(S * 0.92, 48, 0, Math.PI / 5);
    ...
    this.pulse = new THREE.Mesh(ringGeo, ringMat);
  ...
  _buildPit() {
    // светящийся каркас котлована + тёмное дно (STAGE_02)
    const g = new THREE.Group();
    const w = POOL_W + 0.9;
    const d = POOL_D + 0.9;
    const h = POOL_H + 0.35;
  ...
  _buildShellGroup(glbUrl) {
    // чаша: GLB (реалистичная) либо процедурный фолбэк
    ...
    if (glbUrl) {
      ...
      // страховка: если GLB грузится дольше 6с — ставим процедурную, GLB заменит по готовности
      setTimeout(() => {
        if (!this.shellReady && !this.disposed) {
          this._buildProceduralShell();
          finish();
        }
      }, 6000);
  ...
  _buildProceduralShell() {
    ...
    const hole = this._roundedRectShape(POOL_W - 0.36, POOL_D - 0.36, 0.4);
    ...
    // копинг: светлый холодный камень (не душить env — топит материал)
    ...
    coping.position.y = 0.12;
    this.shell.add(coping);
  }

  _buildEquipment() {
    // насос+фильтр+трубы у борта (STAGE_04), гасятся рано — чистые фазы воды/LED
    const g = new THREE.Group();
    ...
    pump.position.set(-POOL_W / 2 - 0.95, -0.62, -0.4);
    g.add(pump);
    const filter = new THREE.Mesh(new THREE.CylinderGeometry(0.26, 0.26, 0.72, 20), metal.clone());
    filter.position.set(-POOL_W / 2 - 0.95, -0.5, 0.45);
    g.add(filter);

    const pipeMat = metal.clone();
    const pipe1 = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.0, 10), pipeMat);
    pipe1.rotation.z = Math.PI / 2;
    pipe1.position.set(-POOL_W / 2 - 0.4, -0.68, -0.4);
    g.add(pipe1);
    const pipe2 = pipe1.clone();
    pipe2.position.z = 0.45;
    g.add(pipe2);

    // узлы-форсунки на стенке
    this.equipNodes = [];
    for (let i = 0; i < 3; i++) {
      const node = new THREE.Mesh(new THREE.SphereGeometry(0.085, 14, 14), accent.clone());
      node.position.set(-POOL_W / 2 + 0.1, -0.78, -0.8 + i * 0.8);
      g.add(node);
      this.equipNodes.push(node);
    }

    this.equipment = g;
    this.equipMats = [metal, filter.material, pipeMat, ...this.equipNodes.map((n) => n.material)];
    this.root.add(g);
  }

  _buildFoam() {
    // ППУ-утепление: тёплая оболочка вокруг чаши (STAGE_05)
    const shape = this._roundedRectShape(POOL_W + 0.28, POOL_D + 0.28, 0.54);
    shape.holes.push(this._roundedRectShape(POOL_W - 0.05, POOL_D - 0.05, 0.46));
    ...
    const foam = new THREE.Mesh(
      new THREE.ExtrudeGeometry(shape, { depth: POOL_H, bevelEnabled: false }),
      this.foamMat
    );
    foam.rotation.x = -Math.PI / 2;
    foam.position.y = -POOL_H + 0.1;
    this.foam = foam;
    this.root.add(foam);
  }
  ...
  _buildWater() {
    ...
    const geo = new THREE.PlaneGeometry(POOL_W - 0.42, POOL_D - 0.42, 48, 24);
    this.water = new THREE.Mesh(geo, this.waterMat);
    this.water.rotation.x = -Math.PI / 2;
    this.water.position.y = -POOL_H;
    this.root.add(this.water);
  }

  _buildLed() {
    // LED-контур на границе воды (не внутри копинга!) + glow-спрайты
    ...
    this.led = new THREE.Mesh(new THREE.TubeGeometry(path, 128, 0.05, 8, true), this.ledMat);
    this.root.add(this.led);

    // мягкое glow-гало вокруг контура
    ...
    this.glowSprites = [];
    const positions = path.getSpacedPoints(24);
  ...
  _buildDust() {
    // фоновые частицы воды/пыли — деликатные, additive
    const count = this.reduced ? 0 : 260;
  ...
  _applyState() {
    ...
    const shellVis = _map(p, 0.25, 0.28);
    const equipT = _map(p, 0.38, 0.45);
    const orbFade = 1 - _map(p, 0.56, 0.615); // гасим обвес рано: чистые фазы воды/LED
    ...
    // блендим с beauty-состоянием героя (вода+LED включены, всё построено)
    const B = (build, hero) => build * (1 - hb) + hero * hb;

    const grid = B(gridT, 0);
    const pit = B(pitT, 0);
    const shellY = B((1 - shellDrop) * 3.6, 0);
    const shellO = B(shellVis, 1);
    ...
    this.pit.visible = pit > 0.01;

    this.shell.position.y = shellY;
    this.shell.visible = shellO > 0.01;
    this.shell.traverse((o) => {
      if (o.isMesh && o.material && 'opacity' in o.material) {
        o.material.transparent = shellO < 0.999;
        o.material.opacity = shellO;
      }
    });

    this.equipMats.forEach((m) => (m.opacity = equip * (m.blending === THREE.AdditiveBlending ? 0.9 : 0.95)));
    this.equipment.visible = equip > 0.01;

    this.foamMat.opacity = foam * 0.16;
    this.foam.visible = foam > 0.01;

    // вода поднимается внутри чаши
    const waterLevel = -POOL_H + 0.14 + water * (POOL_H - 0.3);
    this.water.position.y = waterLevel;
    this.waterMat.uniforms.uOpacity.value = water > 0 ? 0.55 + water * 0.4 : 0;
    this.waterMat.uniforms.uLed.value = led * 0.55;
    this.water.visible = water > 0.01;

    this.ledMat.opacity = led;
    this.led.visible = led > 0.01;
    this.led.position.y = waterLevel + 0.06;
    this.glowMat.opacity = glowK * 0.8;
    this.glowSprites.forEach((s) => (s.position.y = waterLevel + 0.06));

    // копинг: emissive-отклик от LED
    if (this.copingMat) this.copingMat.emissiveIntensity = led * 0.35;

    this.dustMat.opacity = 0.12 + B(readyT, 0.8) * 0.3;

    // bloom дышит с LED, но сдержанно
    ...
    this.root.position.x = wide ? hb * 3.1 : 0;
